...
// 1. Interface update kiya gaya taake ye accountName ko support kare
interface PaymentMethod {
  name: string;
  enabled: boolean;
  details: string;
  accountName?: string; // Naya field add kiya
  imageUrl?: string;
}
...
export default function Wallet() {
  ...
  // 2. State definition mein naya interface use kiya
  ...
  useEffect(() => {
    ...
    const unsubPaymentMethods = onSnapshot(collection(db, 'payment_methods'), (snapshot) => {
      const methods = snapshot.docs.map(doc => ({
        ...
        // 3. YAHAN TABDEELI KI GAYI HAI: accountName fetch kar rahe hain
    ...
    const unsubWithdrawalPaymentMethods = onSnapshot(collection(db, 'withdrawal_payment_methods'), (snapshot) => {
      const methods = snapshot.docs.map(doc => ({
        ...
        enabled: doc.data().enabled,
        details: doc.data().details,
        accountName: doc.data().accountName, // Yahan bhi add kar diya future-proofing ke liye
        imageUrl: doc.data().imageUrl
      }));
