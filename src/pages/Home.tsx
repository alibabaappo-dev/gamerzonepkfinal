...
const handleNextPopup = () => {
  ...
  useEffect(() => {
    ...
    const fetchDashboardData = async () => {
      setLoading(true);
      try {
        const [registrationsSnap, ticketsSnap, transactionsSnap, leaderboardSnap, userNotifsSnap, globalNotifsSnap] = await Promise.all([
        ...
        if (joinedTournamentIds.length > 0) {
          const tournamentsQuery = query(collection(db, 'tournaments'), where(documentId(), 'in', joinedTournamentIds.slice(0, 30)));
          const tournamentsSnap = await getDocs(tournamentsQuery);
          const joinedTournamentsData = tournamentsSnap.docs.map(doc => ({ id: doc.id, ...doc.data() })).filter(t => t.status !== 'completed' && t.status !== 'result' && t.status !== 'Result');
  ...
  return (
    <div className="bg-[#0D0D0D] text-white min-h-screen">
        ...
        <div className="container mx-auto px-8 py-12 relative z-10">
            ...
            <div className="flex items-center gap-4">
              <Link to="/tournaments" className="block">
                <div className="bg-[#1C1C1E]/80 backdrop-blur-md border border-gray-800 p-2 pr-4 rounded-2xl flex items-center gap-4 shadow-xl hover:bg-[#252528] transition-all cursor-pointer group">
                  ...
                  <div>
                    ...
                    <div className="bg-blue-600 text-white text-[10px] font-black px-5 py-1.5 rounded-lg hover:bg-blue-500 transition-colors flex items-center justify-center">Join Now</div>
                  </div>
                </div>
              </Link>
              ...
              <div className="flex items-center gap-6 bg-[#1C1C1E]/50 backdrop-blur-md border border-gray-800/50 px-6 py-2.5 rounded-2xl shadow-lg">
                <div className="flex items-center gap-3">
                  <div className="w-10 h-10 rounded-full bg-gray-800 flex items-center justify-center border border-gray-700 shadow-inner"><User size={20} className="text-gray-400" /></div>
                  <div className="text-left"><p className="text-sm font-bold text-white leading-tight">{user.username}</p><p className="text-[10px] text-gray-500 font-medium">{user.email}</p></div>
                </div>
                <div className="h-8 w-[1px] bg-gray-800/50" />
                <button onClick={onLogout} className="flex items-center gap-2 text-red-500 hover:text-red-400 transition-colors group"><LogOut size={18} className="group-hover:translate-x-1 transition-transform" /><span className="text-sm font-bold">Logout</span></button>
              </div>
            </div>
          </div>
          <div className="grid grid-cols-4 gap-6 mb-12">
            <motion.div whileHover={{ y: -5 }} className="bg-[#1C1C1E] p-6 rounded-3xl border border-gray-800 relative overflow-hidden group hover:border-yellow-400/50 transition-colors shadow-2xl"><div className="absolute top-0 right-0 w-32 h-32 bg-yellow-400/5 rounded-full blur-2xl -mr-16 -mt-16 group-hover:bg-yellow-400/10 transition-colors"></div><div className="flex justify-between items-start mb-8 relative z-10"><div className="bg-yellow-400/10 p-3.5 rounded-2xl group-hover:scale-110 transition-transform duration-300"><Wallet className="text-yellow-400" size={28} /></div><div className="bg-gray-800/50 px-3 py-1 rounded-full border border-gray-700/50"><BarChart2 className="text-yellow-400" size={14} /></div></div><div className="relative z-10"><p className="text-gray-400 text-sm font-medium mb-1">Wallet Balance</p><div className="flex items-baseline gap-2"><span className="text-4xl font-black text-white tracking-tight">{stats.walletBalance}</span><span className="text-gray-500 text-xs font-bold uppercase tracking-wider">coins</span></div></div></motion.div>
            <motion.div whileHover={{ y: -5 }} className="bg-[#1C1C1E] p-6 rounded-3xl border border-gray-800 relative overflow-hidden group hover:border-green-400/50 transition-colors shadow-2xl"><div className="absolute top-0 right-0 w-32 h-32 bg-green-400/5 rounded-full blur-2xl -mr-16 -mt-16 group-hover:bg-green-400/10 transition-colors"></div><div className="flex justify-between items-start mb-8 relative z-10"><div className="bg-green-400/10 p-3.5 rounded-2xl group-hover:scale-110 transition-transform duration-300"><Trophy className="text-green-400" size={28} /></div><div className="bg-gray-800/50 px-3 py-1 rounded-full border border-gray-700/50"><Star className="text-green-400" size={14} /></div></div><div className="relative z-10"><p className="text-gray-400 text-sm font-medium mb-1">Total Wins</p><div className="flex items-baseline gap-2"><span className="text-4xl font-black text-white tracking-tight">{stats.totalWins}</span><span className="text-gray-500 text-xs font-bold uppercase tracking-wider">matches</span></div></div></motion.div>
            <motion.div whileHover={{ y: -5 }} className="bg-[#1C1C1E] p-6 rounded-3xl border border-gray-800 relative overflow-hidden group hover:border-blue-400/50 transition-colors shadow-2xl"><div className="absolute top-0 right-0 w-32 h-32 bg-blue-400/5 rounded-full blur-2xl -mr-16 -mt-16 group-hover:bg-blue-400/10 transition-colors"></div><div className="flex justify-between items-start mb-8 relative z-10"><div className="bg-blue-400/10 p-3.5 rounded-2xl group-hover:scale-110 transition-transform duration-300"><Zap className="text-blue-400" size={28} /></div><div className="bg-gray-800/50 px-3 py-1 rounded-full border border-gray-700/50"><Zap className="text-blue-400" size={14} /></div></div><div className="relative z-10"><p className="text-gray-400 text-sm font-medium mb-1">Total Joined</p><div className="flex items-baseline gap-2"><span className="text-4xl font-black text-white tracking-tight">{stats.activeTournaments}</span><span className="text-gray-500 text-xs font-bold uppercase tracking-wider">matches</span></div></div></motion.div>
            <motion.div whileHover={{ y: -5 }} className="bg-[#1C1C1E] p-6 rounded-3xl border border-gray-800 relative overflow-hidden group hover:border-purple-400/50 transition-colors shadow-2xl"><div className="absolute top-0 right-0 w-32 h-32 bg-purple-400/5 rounded-full blur-2xl -mr-16 -mt-16 group-hover:bg-purple-400/10 transition-colors"></div><div className="flex justify-between items-start mb-8 relative z-10"><div className="bg-purple-400/10 p-3.5 rounded-2xl group-hover:scale-110 transition-transform duration-300"><DollarSign className="text-purple-400" size={28} /></div><div className="bg-gray-800/50 px-3 py-1 rounded-full border border-gray-700/50"><BarChart2 className="text-purple-400" size={14} /></div></div><div className="relative z-10"><p className="text-gray-400 text-sm font-medium mb-1">Total Earnings</p><div className="flex items-baseline gap-2"><span className="text-4xl font-black text-white tracking-tight">+{stats.totalEarnings}</span><span className="text-gray-500 text-xs font-bold uppercase tracking-wider">coins</span></div></div></motion.div>
          </div>
          ...
          <div className="grid grid-cols-12 gap-8">
            <div className="col-span-8 space-y-8">
              <div className="bg-[#1C1C1E] rounded-3xl border border-gray-800 overflow-hidden shadow-2xl">
                <div className="p-8 border-b border-gray-800/50 flex justify-between items-center bg-[#1F1F22]/50">
                  ...
                  <Link to="/tournaments" className="text-yellow-400 text-sm font-bold flex items-center hover:text-yellow-300 transition-colors bg-yellow-400/10 px-4 py-2 rounded-xl hover:bg-yellow-400/20">View All <ChevronRight size={16} className="ml-1" /></Link>
                </div>
                <div className="p-6">
              ...
              <div className="bg-[#1C1C1E] rounded-3xl border border-gray-800 overflow-hidden shadow-2xl">
                <div className="p-8 border-b border-gray-800/50 flex justify-between items-center bg-[#1F1F22]/50">
                  <div className="flex items-center space-x-4"><div className="bg-blue-400/10 p-2 rounded-xl"><BarChart2 className="text-blue-400" size={20} /></div><div><h2 className="text-xl font-bold text-white">Recent Transactions</h2><p className="text-xs text-gray-500 font-medium mt-0.5">Your financial history</p></div></div>
                  <Link to="/transactions" className="text-blue-400 text-sm font-bold flex items-center hover:text-blue-300 transition-colors bg-blue-400/10 px-4 py-2 rounded-xl hover:bg-blue-400/20">View All <ChevronRight size={16} className="ml-1" /></Link>
                </div>
                <div className="p-6">
                  {recentTransactions.length > 0 ? (<div className="space-y-3">{recentTransactions.map((tx) => (<div key={tx.id} className="flex items-center justify-between p-4 rounded-2xl bg-[#252528] border border-gray-700/30 hover:bg-[#2A2A2D] transition-colors group"><div className="flex items-center space-x-4"><div className={`p-3 rounded-xl ${tx.type === 'Deposit' || tx.type === 'Winning' ? 'bg-green-500/10 text-green-400' : 'bg-red-500/10 text-red-400'}`}><Zap size={20} /></div><div><p className="font-bold text-white group-hover:text-gray-200 transition-colors">{tx.type}</p><p className="text-gray-500 text-xs font-medium mt-0.5">{tx.date}</p></div></div><div className="text-right"><p className={`font-black text-lg ${tx.type === 'Deposit' || tx.type === 'Winning' ? 'text-green-400' : 'text-red-400'}`}>{tx.type === 'Deposit' || tx.type === 'Winning' ? '+' : '-'}{tx.amount}</p><span className={`text-[10px] font-bold px-2 py-0.5 rounded uppercase tracking-wide ${tx.status === 'Completed' ? 'bg-green-500/10 text-green-400' : tx.status === 'Pending' ? 'bg-yellow-500/10 text-yellow-400' : 'bg-red-500/10 text-red-400'}`}>{tx.status}</span></div></div>))}</div>) : (<div className="text-center py-12 bg-[#252528]/50 rounded-2xl border border-dashed border-gray-800"><p className="text-gray-500 font-medium">No recent transactions found</p></div>)}
                </div>
              </div>
            </div>
            <div className="col-span-4 space-y-8">
              ...
              <div className="bg-[#1C1C1E] rounded-3xl border border-gray-800 overflow-hidden shadow-2xl">
                <div className="p-6 border-b border-gray-800/50 flex justify-between items-center bg-[#1F1F22]/50"><div className="flex items-center space-x-3"><Bell className="text-yellow-400" size={18} /><h2 className="font-bold text-white">Notifications</h2></div><Link to="/notifications" className="text-yellow-400 text-xs font-bold hover:underline">View All</Link></div>
                <div className="p-6 space-y-4">
                  {recentNotifications.length > 0 ? (recentNotifications.map((notif) => (<div key={notif.id} className="bg-[#252528] p-4 rounded-2xl border border-gray-700/30 hover:bg-[#2A2A2D] transition-colors cursor-pointer group"><div className="flex justify-between items-start mb-2"><span className={`w-2 h-2 rounded-full mt-1.5 ${notif.isRead ? 'bg-gray-600' : 'bg-yellow-400'}`}></span><p className="text-[10px] text-gray-500 font-medium">{notif.createdAt?.toDate ? notif.createdAt.toDate().toLocaleTimeString([], { hour: '2-digit', minute: '2-digit' }) : 'Just now'}</p></div><p className="text-sm font-bold text-gray-200 group-hover:text-white transition-colors pl-4 line-clamp-1">{notif.title}</p><p className="text-xs text-gray-500 pl-4 mt-1 line-clamp-1">{notif.message}</p></div>))) : (<div className="text-center py-8"><p className="text-gray-500 text-xs">No recent notifications</p></div>)}
                </div>
              </div>
              <div className="bg-[#1C1C1E] rounded-3xl border border-gray-800 overflow-hidden shadow-2xl relative">
                ...
                <div className="p-6 border-b border-gray-800/50 flex justify-between items-center bg-[#1F1F22]/50 relative z-10"><div className="flex items-center space-x-3"><Trophy className="text-purple-400" size={18} /><h2 className="font-bold text-white">Leaderboard</h2></div><Link to="/leaderboard" className="text-purple-400 text-xs font-bold hover:underline">View All</Link></div>
                <div className="p-8 text-center relative z-10">
                  <div className="inline-block relative"><p className="text-6xl font-black text-white mb-2 tracking-tighter drop-shadow-lg">#{userRank || '?'}</p>{userRank > 0 && userRank <= 100 && (<div className="absolute -top-4 -right-6 bg-purple-500 text-white text-[10px] font-bold px-2 py-0.5 rounded-full uppercase tracking-wide animate-bounce">Top 100</div>)}</div>
                  <p className="text-xs text-gray-500 uppercase tracking-widest font-bold mb-8">Current Rank</p>
                  <Link to="/leaderboard" className="block bg-gradient-to-r from-purple-600 to-blue-600 hover:from-purple-500 hover:to-blue-500 text-white font-bold py-4 rounded-2xl w-full transition-all shadow-lg shadow-purple-500/20 hover:shadow-purple-500/40 hover:-translate-y-0.5 text-center">View Full Rankings</Link>
                </div>
              </div>
              <div className="bg-[#1C1C1E] rounded-3xl border border-gray-800 overflow-hidden shadow-2xl">
                <div className="p-6 border-b border-gray-800/50 flex justify-between items-center bg-[#1F1F22]/50"><div className="flex items-center space-x-3"><MessageSquare className="text-blue-400" size={18} /><h2 className="font-bold text-white">Support</h2></div><Link to="/support" className="text-blue-400 text-xs font-bold hover:underline">View All</Link></div>
                <div className="p-6 space-y-4">
                  {supportTickets.length > 0 ? (supportTickets.map((req) => (<div key={req.id} className="flex items-center justify-between p-4 rounded-2xl bg-[#252528] border border-gray-700/30 hover:bg-[#2A2A2D] transition-colors"><div className="flex items-center space-x-4"><div className="bg-blue-900/20 p-2.5 rounded-xl"><MessageSquare className="text-blue-400" size={18} /></div><div><p className="font-bold text-white text-sm">{req.subject}</p><p className="text-gray-500 text-[10px] font-medium mt-0.5">#{req.id.slice(-4)}</p></div></div><span className={`text-[10px] font-bold px-2.5 py-1 rounded-lg uppercase tracking-wide ${req.status === 'Pending' ? 'bg-yellow-400/10 text-yellow-400 border border-yellow-400/20' : req.status === 'Solved' ? 'bg-green-400/10 text-green-400 border border-green-400/20' : 'bg-blue-400/10 text-blue-400 border border-blue-400/20'}`}>{req.status}</span></div>))) : (<div className="text-center py-6"><p className="text-gray-500 text-sm font-medium">No recent support requests</p></div>)}
                  <Link to="/support" className="block w-full bg-[#252528] text-center text-gray-300 hover:text-white font-bold py-3 rounded-xl hover:bg-gray-700 transition-colors mt-2 text-sm border border-gray-700/50">Create New Request</Link>
                </div>
              </div>
            </div>
          </div>
      ...
      <div className="lg:hidden container mx-auto p-4 pb-24">
        ...
        <Card className="mb-6 border-green-400/40"><div className="flex justify-between items-center mb-4"><h2 className="font-bold text-lg">Recent Transactions</h2><Link to="/transactions" className="text-yellow-400 text-sm font-semibold flex items-center">View All <ChevronRight size={16} /></Link></div><div className="space-y-4">{recentTransactions.length > 0 ? (recentTransactions.map((tx) => (<div key={tx.id} className="flex items-center justify-between"><div className="flex items-center"><div className={`w-10 h-10 rounded-full flex items-center justify-center mr-3 ${tx.type === 'Deposit' || tx.type === 'Winning' ? 'bg-green-500/10' : 'bg-red-500/10'}`}><Zap size={20} className={tx.type === 'Deposit' || tx.type === 'Winning' ? 'text-green-400' : 'text-red-400'}/></div><div><p className="font-semibold">{tx.type}</p><p className="text-xs text-gray-400">{tx.date}</p></div></div><div><p className={`font-bold ${tx.type === 'Deposit' || tx.type === 'Winning' ? 'text-green-400' : 'text-red-400'}`}>{tx.type === 'Deposit' || tx.type === 'Winning' ? '+' : '-'}{tx.amount} coins</p><p className="text-xs text-gray-500 text-right">{tx.status}</p></div></div>))) : (<div className="text-center py-4"><p className="text-gray-500 text-xs">No recent transactions</p></div>)}</div></Card>
        <Card className="mb-6 border-yellow-600/80 "><div className="flex flex-col items-center text-center"><div className="w-20 h-20 rounded-full bg-yellow-400/10 flex items-center justify-center border-2 border-yellow-400 mb-4"><User size={40} className="text-yellow-400"/></div><h3 className="font-bold text-xl">{user.username}</h3><p className="text-gray-400 text-sm">{user.email}</p><div className="flex space-x-8 mt-4"><div><p className="text-2xl font-bold">{stats.activeTournaments}</p><p className="text-xs text-gray-500">Tournaments Joined</p></div><div><p className="text-2xl font-bold">{stats.totalWins}</p><p className="text-xs text-gray-500">Wins</p></div></div></div></Card>
        <Card className="mb-6 border-blue-600/80"><div className="flex justify-between items-center mb-4"><div className="flex items-center space-x-3"><Bell size={20} className="text-blue-400" /><h2 className="font-bold text-lg">Notifications</h2></div><Link to="/notifications" className="text-blue-400 text-sm font-semibold flex items-center">View All <ChevronRight size={16} /></Link></div><div>{recentNotifications.length > 0 ? (<div className="space-y-3">{recentNotifications.map((notif) => (<div key={notif.id}><p className="font-semibold text-sm line-clamp-1">{notif.title}</p><p className="text-xs text-gray-400">{notif.createdAt?.toDate ? notif.createdAt.toDate().toLocaleString() : 'Just now'}</p></div>))}</div>) : (<p className="text-gray-500 text-xs">No recent notifications</p>)}</div></Card>
        <Card className="mb-6 border-purple-600/80 "><div className="flex justify-between items-center mb-4"><div className="flex items-center space-x-3"><Trophy size={20} className="text-purple-400" /><h2 className="font-bold text-lg">Leaderboard</h2></div><Link to="/leaderboard" className="text-purple-400 text-sm font-semibold flex items-center">View All <ChevronRight size={16} /></Link></div><div className="text-center"><p className="text-4xl font-bold text-purple-400">#{userRank || '?'}</p><p className="text-sm text-gray-400 mb-4">Current Rank</p><div className="flex justify-around items-center mb-4"><div><p className="text-2xl font-bold">{stats.totalWins}</p><p className="text-xs text-gray-500">Wins</p></div><div><p className="text-2xl font-bold">0</p><p className="text-xs text-gray-500">Streak</p></div></div><Link to="/leaderboard" className="block bg-purple-500/80 text-white font-bold py-2.5 px-6 rounded-lg w-full text-center">View Rankings</Link></div></Card>
      ...
      <AnimatePresence>
        {showProfileModal && (
          <div className="fixed inset-0 z-[200] flex items-center justify-center p-4 bg-black/60 backdrop-blur-2xl"><motion.div initial={{ scale: 0.9, opacity: 0, y: 20 }} animate={{ scale: 1, opacity: 1, y: 0 }} className="bg-[#1C1C1E]/80 w-full max-w-[280px] rounded-[2rem] border border-gray-800/50 shadow-2xl overflow-hidden"><div className="p-6"><div className="flex flex-col items-center text-center mb-5"><div className="w-14 h-14 bg-yellow-400/10 rounded-2xl flex items-center justify-center mb-3 border border-yellow-400/20"><User size={28} className="text-yellow-400" /></div><h2 className="text-lg font-black text-white uppercase tracking-tight">Profile Setup</h2><p className="text-gray-500 text-[10px] mt-1 font-medium">Complete your profile to continue.</p></div><form onSubmit={async (e) => { e.preventDefault(); if (!profileUsername.trim() || profilePhone.length !== 10) return; try { setIsUpdatingProfile(true); await updateDoc(doc(db, 'users', user.uid), { username: profileUsername.trim(), phoneNumber: '+92' + profilePhone.trim() }); setShowProfileModal(false); } catch (err) { console.error('Error updating profile:', err); } finally { setIsUpdatingProfile(false); } }} className="space-y-3.5"><div className="space-y-1"><label className="text-[8px] font-black uppercase tracking-widest text-gray-500 ml-1">Username</label><div className="relative group"><div className="absolute inset-y-0 left-0 pl-3 flex items-center pointer-events-none text-gray-500 group-focus-within:text-yellow-400 transition-colors"><User size={14} /></div><input type="text" required value={profileUsername} onChange={(e) => setProfileUsername(e.target.value)} placeholder="Username" className="w-full bg-black/40 border border-gray-800 rounded-xl pl-9 pr-3 py-2.5 text-xs text-white focus:border-yellow-400 outline-none transition-all font-bold" /></div></div><div className="space-y-1"><label className="text-[8px] font-black uppercase tracking-widest text-gray-500 ml-1">Phone Number</label><div className="relative group"><div className="absolute inset-y-0 left-0 pl-3 flex items-center pointer-events-none"><span className="text-[10px] font-black text-yellow-400">+92</span></div><input type="tel" required maxLength={10} value={profilePhone} onChange={(e) => { const val = e.target.value.replace(/\D/g, ''); if (val.length <= 10) setProfilePhone(val); }} placeholder="3001234567" className="w-full bg-black/40 border border-gray-800 rounded-xl pl-11 pr-3 py-2.5 text-xs text-white focus:border-yellow-400 outline-none transition-all font-bold" /></div><div className="flex justify-between items-center px-1"><p className="text-[7px] text-gray-600">10 digits (e.g. 300...)</p><p className={`text-[7px] font-bold ${profilePhone.length === 10 ? 'text-green-500' : 'text-gray-600'}`}>{profilePhone.length}/10</p></div></div><button type="submit" disabled={isUpdatingProfile || !profileUsername.trim() || profilePhone.length !== 10} className="w-full bg-yellow-400 hover:bg-yellow-500 disabled:opacity-30 text-black font-black py-3.5 rounded-xl text-[10px] uppercase tracking-widest transition-all shadow-lg shadow-yellow-400/10 flex items-center justify-center gap-2 mt-2">{isUpdatingProfile ? (<Loader2 size={14} className="animate-spin" />) : (<><Check size={14} /> Save & Continue</>)}</button></form></div></motion.div></div>
        )}
      </AnimatePresence>
    </div>
  );
