...
// --- Re-usable Components (UI 100% SAME) ---
      ...
      <div className="fixed inset-0 z-[500] flex items-center justify-center p-4 bg-black/90 backdrop-blur-sm">
        ...
        >
          ...
          <div className="p-8 pt-10 flex flex-col items-center">
            ...
            <h2 className="text-sm font-black text-center text-yellow-500 uppercase tracking-tight mb-6 px-2">
              {title}
            </h2>
...
export default function Home({ user, onLogout }) {
  ...
  // 2. OPTIMIZED DASHBOARD FETCH (PREVENTS 191M READS)
  useEffect(() => {
    ...
    const fetchDashboardData = async () => {
      setLoading(true);
      try {
        const [regSnap, tktSnap, txSnap, leaderSnap] = await Promise.all([
        ...
        const allTournamentIds = regSnap.docs.map(doc => doc.data().tournamentId);
        
        if (allTournamentIds.length > 0) {
          const tSnap = await getDocs(query(collection(db, 'tournaments'), where(documentId(), 'in', allTournamentIds.slice(0, 10))));
          const tournamentData = tSnap.docs.map(doc => ({ id: doc.id, ...doc.data() }));
          
          const activeOnly = tournamentData.filter(t => t.status !== 'completed' && t.status !== 'Result' && t.status !== 'result');
          
          setJoinedTournaments(activeOnly);
          setStats(prev => ({ 
            ...prev, 
            totalEarnings: earnings, 
            activeTournaments: allTournamentIds.length 
          }));
        } else {
          setJoinedTournaments([]);
          setStats(prev => ({ ...prev, totalEarnings: earnings, activeTournaments: 0 }));
        }
  ...
  return (
    <div className="bg-[#0D0D0D] text-white min-h-screen">
      ...
      <div className="hidden lg:block min-h-screen bg-[#0D0D0D] relative overflow-hidden">
        ...
        <div className="container mx-auto px-8 py-12 relative z-10">
          ...
          <div className="grid grid-cols-4 gap-6 mb-12">
            ...
            <StatCard icon={<Zap className="text-blue-400" size={28} />} title="Total Joined" value={stats.activeTournaments} subtitle="history" color="blue" trendIcon={<Zap size={14} className="text-blue-400" />} />
            ...
            <div className="col-span-8 space-y-8">
              <div className="bg-[#1C1C1E] rounded-3xl border border-gray-800 overflow-hidden shadow-2xl">
                <div className="p-8 border-b border-gray-800/50 flex justify-between items-center bg-[#1F1F22]/50">
                  <div className="flex items-center space-x-4"><div className="bg-yellow-400/10 p-2 rounded-xl"><Trophy className="text-yellow-400" size={20} /></div><div><h2 className="text-xl font-bold text-white">Current Joined Matches</h2><p className="text-xs text-gray-500 font-medium mt-0.5">Your active tournaments</p></div></div>
      ...
      {/* Mobile Dashboard (UI PRESERVED & LINKS FIXED) */}
      <div className="lg:hidden container mx-auto p-4 pb-24">
        ...
        <div className="grid grid-cols-2 gap-4 mb-8 pl-2 pr-2">
          ...
          <StatCard icon={<Zap size={20} className="text-blue-400" />} title="Total Joined" value={stats.activeTournaments} subtitle="all-time" color="blue" />
        ...
        <div className="grid grid-cols-2 gap-4 mb-8 pl-2 pr-2">
          <Link to="/tournaments"><NavCard icon={<Trophy size={24} className="text-yellow-400" />} title="Matches" /></Link>
        ...
        {joinedTournaments.length > 0 && (
          <Card className="mb-6 border-yellow-400/80">
            <h2 className="font-bold text-lg mb-4 flex items-center gap-2"><Trophy size={20} className="text-yellow-400" /> Current Matches</h2>
            {joinedTournaments.map((t) => (
              <div key={t.id} className="bg-[#1C1C1E] p-4 rounded-xl border border-gray-700 mb-3">
                <div className="flex justify-between items-start mb-2"><h3 className="font-bold text-white text-sm">{t.name}</h3><span className="text-green-400 text-xs font-bold">{t.prizePool} coins</span></div>
                <Link to="/tournaments" className="block w-full bg-yellow-400 text-black text-center text-xs font-bold py-2 rounded-lg">View Match</Link>
              </div>
            ))}
          </Card>
        )}
     ...
     {/* SEQUENTIAL POPUPS (SESSION BASED) */}
      <EmojiModal isOpen={activePopup === 1} onClose={handleNextPopup} title="⚡ NEW MATCHES ADDED ✅">
        <div className="space-y-3">
          ...
          <div className="pt-4 border-t border-gray-800"><p className="text-[11px] font-bold text-gray-400 leading-relaxed">💪 Ab Kam Entry Me Zyada Prize !<br />⚡ Slots Rehte Hain Join Now !</p><p className="text-[10px] mt-4 text-yellow-400 font-black uppercase tracking-tight">GAMER ZONE — JOIN NOW ⚔️🔥</p></div>
        </div>
      </EmojiModal>

      <EmojiModal isOpen={activePopup === 2} onClose={handleNextPopup} title="WATCH FULL VIDEO">
        <div className="space-y-4">
          <p className="text-[10px] font-bold text-gray-200 px-1 leading-relaxed">🚀 Watch Guide On Youtube, Tiktok & Join WhatsApp Group!</p>
          <div className="space-y-3 pt-4 border-t border-gray-800 px-1">
      ...
      <EmojiModal isOpen={activePopup === 3} onClose={handleNextPopup} title="🚨 BAN ALERT 🚨">
        <div className="space-y-3 text-xs font-bold text-red-500 px-1">
          <p>⚠️ FAKE DEPOSIT = BAN!</p>
          <p>⚠️ SPAM TICKETS = DEVICE BAN!</p>
          <p className="text-[10px] mt-4 text-yellow-400 font-black uppercase text-center pt-2 border-t border-gray-800">🚨 NO SECOND CHANCES 🚨</p>
        </div>
      </EmojiModal>
      ...
      <AnimatePresence>
        {showProfileModal && (
          <div className="fixed inset-0 z-[600] flex items-center justify-center p-4 bg-black/80 backdrop-blur-xl">
             <div className="bg-[#1C1C1E] p-8 rounded-[2.5rem] w-full max-w-sm border border-gray-800 text-center">
