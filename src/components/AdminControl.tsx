...
export default function AdminControl({ users, currentUser }: { users: any[], currentUser: any }) {
  ...
  const PermissionsUI = ({ isMobile = false }: { isMobile?: boolean }) => (
    // FIX 1: Added min-h-0 to the main flex container
    <div className={`${isMobile ? 'p-4' : 'bg-[#131B2F] border border-gray-800 rounded-3xl p-5 shadow-2xl'} h-full flex flex-col min-h-0`}>
      
      {/* FIX 2: Added shrink-0 to header so it doesn't get squished */}
      <div className="flex flex-col xl:flex-row items-start xl:items-center justify-between gap-4 mb-5 pb-5 border-b border-gray-800/50 shrink-0">
      ...
      {/* FIX 3: Added min-h-0 so the inner scroll area calculates height properly */}
      <div className="flex-1 overflow-y-auto pr-2 custom-scrollbar min-h-0 pb-4">
        <div className="flex items-center gap-1.5 mb-3 sm:mb-4 shrink-0">
  ...
  return (
    <div className="grid lg:grid-cols-12 gap-8">
      <div className="lg:col-span-4 flex flex-col h-[70vh] min-h-[600px] lg:h-[75vh]">
        <div className="bg-[#131B2F] border border-gray-800 rounded-3xl p-2 flex flex-col h-full shadow-xl">
          <div className="space-y-3 sm:space-y-4 mb-3 sm:mb-4 shrink-0">
      ...
      {/* Desktop view */}
      <div className="hidden lg:flex flex-col lg:col-span-8 min-h-0 h-full">
        {selectedUser ? (
          <PermissionsUI />
        ) : (
      ...
      <AnimatePresence>
        {isModalOpen && selectedUser && (
          <div className="lg:hidden fixed inset-0 z-[110] flex items-end sm:items-center justify-center p-3">
            ...
            >
              <div className="w-10 h-1 bg-gray-800/50 rounded-full mx-auto mt-3 mb-1 sm:hidden shrink-0" />
              
              {/* FIX 4: Added shrink-0 to modal header */}
              <div className="flex justify-between items-center px-6 py-4 border-b border-gray-800/50 shrink-0">
              ...
              {/* FIX 5: Changed to flex flex-col and min-h-0 for proper scroll calculation */}
              <div className="flex-1 overflow-hidden min-h-0 flex flex-col">
                <PermissionsUI isMobile={true} />
              </div>
              
              {/* FIX 6: Added shrink-0 to footer */}
              <div className="p-5 bg-[#0B1121] border-t border-gray-800/50 shrink-0">
                <button 
                  onClick={() => setIsModalOpen(false)}
