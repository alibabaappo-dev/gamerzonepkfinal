...
interface BuyCoinsModalProps {
  ...
  minDeposit?: number;
  // Interface mein accountName shamil kar diya gaya hai
  paymentMethods?: { name: string; enabled: boolean; details: string; accountName?: string; imageUrl?: string }[];
}
...
export default function BuyCoinsModal({ isOpen, onClose, initialAmount = '', onSubmit, minDeposit = 50, paymentMethods = [] }: BuyCoinsModalProps) {
  ...
  }, [isOpen, initialAmount, paymentMethods]); // Added paymentMethods to dependency array to be safe
  ...
  // Account Number (details) fetch karne ka function
  const getPaymentNumber = (method: string) => {
    return paymentMethods.find(m => m.name === method)?.details || 'Not Available';
  };

  // Account Name fetch karne ka naya function
  const getAccountName = (method: string) => {
    return paymentMethods.find(m => m.name === method)?.accountName || 'Not Available';
  };
      ...
      <div className="bg-[#0D0D0D] border border-gray-800 rounded-[2rem] w-full max-w-md max-h-[90vh] overflow-y-auto text-white font-sans animate-in fade-in zoom-in duration-300 shadow-[0_0_50px_rgba(0,0,0,0.5)]">
        <div className="p-8">
          ...
          <div className="space-y-6">
            ...
            {/* Transfer To (Account Number) Box */}
            <div className="bg-[#1A1A1A] border border-gray-800 rounded-2xl p-6 relative overflow-hidden pb-8"> 
            ...
            {/* NAYA BOX: Account Name ke liye (Bilkul same UI) */}
            <div className="bg-[#1A1A1A] border border-gray-800 rounded-2xl p-6 relative overflow-hidden pb-8"> 
              <div className="flex justify-between items-start"> 
                <div> 
                  <p className="text-gray-500 text-[10px] font-black uppercase tracking-widest mb-2">ACCOUNT NAME</p> 
                  <p className="text-2xl font-black text-white tracking-[0.1em]">{getAccountName(paymentMethod)}</p> 
                </div> 
              </div> 
            </div>
